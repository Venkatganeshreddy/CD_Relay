...
// 'Mar-27' / 'Aug-26' / 'July-26' / 'Jul 2026' / '2026-07' → sortable 'YYYY-MM'.
// Takes the first 3 letters of the month token, so full month names normalize too.
const toPeriod = (s) => {
  s = String(s || '').trim();
  if (/^\d{4}-\d{2}$/.test(s)) return s;
  const m = s.toLowerCase().match(/^([a-z]{3})[a-z]*[\s\-\/]*'?(\d{2,4})$/);
  return m && MONTHS[m[1]] ? `${m[2].length === 2 ? '20' + m[2] : m[2]}-${MONTHS[m[1]]}` : s;
};
