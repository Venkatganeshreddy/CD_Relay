...
(function () {
  ...
  const CODEX_WORKFLOWS = [
    ...
    { id: 'wf-task',    name: 'Task flow',             trigger: 'User clicks Create task · 6:00 PM snapshot', agents: ['(no LLM — structured form)'], outputs: ['tasks', 'worklogs'], version: 'v1',
      objective: 'Capture work in the CD Task-flow format with auto-mapped Metric and Task categories.',
      steps: [
        { n: 1, title: 'Open Create task', detail: 'Structured form. EMP ID auto-filled from the signed-in owner (or pick another owner).', done: true },
        { n: 2, title: 'Product-Audience + Stack', detail: 'Product-Audience is multi-select and required; Stack is multi-select and optional.', done: true },
        { n: 3, title: 'Output Category → auto Metric & Task category', detail: 'Choosing an Output Category auto-derives Metric Category, Activity Category and Task Category from the v.11 mapping table.', done: true },
        { n: 4, title: 'Output Count + Task template', detail: 'Output Count is optional (0 allowed; N/A for Executive Ops & Business Impact metrics). The per-Task-category template (the “Task”) is optional.', done: true },
        { n: 5, title: 'Status + Due date', detail: 'Status (In-progress / Done / Blocked / Overdue / Backlog) and Due date are mandatory. A reason is required when Blocked or Overdue.', done: true },
        { n: 6, title: 'Create — task saved', detail: 'Task is stored with its auto Metric/Task category and shown on the task board and the dashboard “Captured work” table in the daily format.', done: true },
        { n: 7, title: '6:00 PM snapshot', detail: 'Each evening the owner reviews their open + backlog tasks, changes only the status, and notes the backlog. Acknowledgement records lastAckDate.', done: true },
        { n: 8, title: 'Feeds the Escalation flow', detail: 'Blocked / overdue / unacknowledged tasks are picked up by the Escalation flow.', done: true },
      ] },
    { id: 'wf-escalation', name: 'Escalation flow',    trigger: 'Task blocked/overdue · pg_cron every 30 min · 6:00 PM check-in', agents: ['Sentry'], outputs: ['notifications', 'escalations'], version: 'v2',
      objective: 'Track blocked/overdue tasks and escalate through the reporting hierarchy.',
      steps: [
        { n: 1, title: 'Task marked as Blocked', detail: 'Notifies the immediate reporting manager (and the originator).', done: true },
        { n: 2, title: 'Task remains blocked (> 1 day)', detail: 'Escalates to the next hierarchy level, climbing one level per scan (L1 → L2 → L3).', done: true },
        { n: 3, title: 'Task crosses due date (> 2 days)', detail: 'Sends an overdue trigger to the uploader / originator.', done: true },
        { n: 4, title: 'In-progress > 2 days, or unacknowledged at 6:00 PM', detail: 'Crosses the threshold and escalates up the chain via the same engine.', done: true },
        { n: 5, title: 'Escalated tasks shown separately', detail: 'An L3/L2 sees escalated/blocked/overdue items in the dedicated Escalated tab on the task board.', done: true },
        { n: 6, title: 'Server-side auto-scan', detail: 'Runs every 30 min via pg_cron (run_escalations + run_overdue_triggers); the 6:00 PM job (run_daily_acknowledgement) drives the unacknowledged path.', done: true },
      ] },
